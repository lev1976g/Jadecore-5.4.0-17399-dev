...
namespace Movement
{
    ...
    void MoveSplineInit::Launch()
    {
        ...
        // Writes bits
        data.WriteBit(hasUnk1);                     // 176
        data.WriteBit(moverGUID[5]);                // 21
        data.WriteBit(!splineflags.animation);      // 72
        data.WriteBit(false);                       // 80
        data.WriteBit(moverGUID[4]);                // 20
        data.WriteBit(moverGUID[3]);                // 19
        data.WriteBit(true);                        // 76
        data.WriteBit(seat == -1);                  // 120
        data.WriteBit(moverGUID[2]);                // 18
        data.WriteBit(sendSplineFlags == 0);        // 64
        data.WriteBit(moverGUID[0]);                // 16

        if (hasUnk1)                                // if 176
        {
            data.WriteBits(0, 2);                   // 156
            data.WriteBits(unkCounter, 22);         // sub
        }

        data.WriteBits(packedWPcount, 22);          // sub
        data.WriteBit(!hasUnk2);                    // 109
        data.WriteBit(moverGUID[7]);                // 23
        ...
        data.WriteBit(!splineflags.animation);      // 69
        data.WriteBit(!splineflags.parabolic);      // 88
        data.WriteBit(!splineflags.parabolic);      // 84
        data.WriteBit(moverGUID[6]);                // 22
        data.WriteBits(WPcount, 20);                // sub
        data.WriteBit(moverGUID[1]);                // 17
        data.WriteBit(!hasUnk3);                    // 108
        data.WriteBits(splineType, 3);              // 68

        if (splineType == MonsterMoveFacingTarget)  //if v9 == 3
        {
        ...
        if (hasUnk1) // if 176
        {
            ...
            for (uint32 i = 0; i < unkCounter; i++) // if v3 + 140
            {
                data << uint16(0); // 144 + 2
                data << uint16(0); // a2++
            }
        ...
        if (splineType == MonsterMoveFacingTarget) // if 68 == 3
        {
            ObjectGuid facingTargetGUID = move_spline.facing.target;
        ...
        data << float(move_spline.spline.getPoint(move_spline.spline.first()).y); //sub 
        data.WriteByteSeq(moverGUID[7]); // 28

        if (splineflags.animation) // 23
            data << int32(move_spline.effect_start_time);

        if ((splineflags & MoveSplineFlag::UncompressedPath) == 0) // if 72
        {
        ...
        data << float(0.0f);                    // 44
        data << float(move_spline.spline.getPoint(move_spline.spline.first()).z); // 32
        data << float(0.0f);                    // 48

        if (splineType == MonsterMoveFacingSpot) // 68 / 200 / 192 / 196
            data << move_spline.facing.f.z << move_spline.facing.f.x << move_spline.facing.f.y;

        if (splineflags & MoveSplineFlag::UncompressedPath) // if 92
        {
        ...
        data.WriteByteSeq(moverGUID[5]); // 21
        data << uint32(getMSTime());            // 40

        if (sendSplineFlags) // 64
            data << uint32(sendSplineFlags);

        if (splineflags.animation) // 69
            data << uint8(splineflags.getAnimationId());

        data.WriteByteSeq(moverGUID[0]); // 16

        if (hasUnk2) // 109
            data << uint8(0);                   // unk byte

        if (splineflags.parabolic) // 88
            data << move_spline.effect_start_time;

        data << float(move_spline.spline.getPoint(move_spline.spline.first()).x); // 24
        data << move_spline.Duration(); // 80
        data.WriteByteSeq(moverGUID[4]); // 20

        if (splineflags.parabolic) // 84
            data << move_spline.vertical_acceleration;

        if (seat != -1) // 120
            data << int8(seat);

        if (splineType == MonsterMoveFacingAngle) // 120
            data << move_spline.facing.angle;

        if (hasUnk3) //68
            data << uint8(0);                   // unk byte

        data << float(0.0f); // 52                     // unk float

        if (false) // 76
            data << uint32(0);                  // unk uint32

        data.WriteByteSeq(moverGUID[6]); // 22
        data.WriteByteSeq(moverGUID[2]); // 18
        data.WriteByteSeq(moverGUID[3]); // 19
        data.WriteByteSeq(moverGUID[1]); // 17
